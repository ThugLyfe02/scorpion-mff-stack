...
from .hac_edge import HACEdgeReport, evaluate_selected_hac
...
def run_quant_audit(trades: tuple[CompletedTrade, ...]) -> QuantAuditReport:
    ...
    hac_edge = evaluate_selected_hac(segments, selected_set)
    ...
    hac_by_segment = {item.segment: item for item in hac_edge}
    qualified = tuple(
        name
        ...
        and (name not in hac_by_segment or hac_by_segment[name].qualified)
    )
    failures: list[str] = []
    ...
    failures.extend(
        f"hac_edge:{item.segment}:{failure}"
        for item in hac_edge
        if not item.qualified
        for failure in item.failures
    )
...
def quant_audit_main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Run portfolio/tail clustering, heavy-tail and serial-correlation robustness, "
            "hierarchical shrinkage, and selection-adjusted diagnostics on certified "
            "CompletedTrade JSONL. Research-only."
        )
    )
